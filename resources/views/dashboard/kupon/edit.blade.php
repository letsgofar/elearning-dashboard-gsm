...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                <h4 class="card-title">EditKupon Perubahan</h4>
                <p class="card-description">
                    Admin bisa mengedit kupon dengan mengisi form berikut
                </p>
                <form class="forms-sample" name="add_kupon_form" id="add_kupon_form" method="POST" action="/kupon/{{ $coupons->slug }}/edit">
                    @method('patch')
                    @csrf
                    <div class="form-group">
                    <label for="">Nama Kupon</label>
                    <input type="text" class="form-control" id="coupon_title" name="nama" placeholder="Nama Kupon" value="{{old('nama') ?? $coupons->nama}}" required>
                    </div>
                    <div class="form-group">
                        <label for="">Kode Kupon</label>
                        <input type="text" class="form-control" id="coupon_code" name="code" placeholder="Kode Kupon" value="{{old('nama') ?? $coupons->code}}" required>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            ...
                            <label for="">Date</label>
                                <input type="date" class="form-control" name="date" placeholder="dd-mm-yyyy" value="{{old('nama') ?? $coupons->date}}">
                            </div>
                        </div>
                        <div class="col-sm-5">
                            <div class="form-group" id="kuotaView" style="display:none;">
                                <label for="">Kuota</label>
                                    <input type="text" class="form-control" name="kuota" placeholder="Masukkan Kuota" value="{{old('nama') ?? $coupons->kuota}}">
                            </div>
                        </div>
                    </div>
                    <button class="btn btn-primary mr-2" type="submit">Update Kupon</button>
                </form>
                </div>
            </div>
            </div>
