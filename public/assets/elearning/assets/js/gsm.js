...
function checkScored(arr) { 
    return arr == localStorage.getItem("idmodulspesial");
}
...
$.ajax({
    ...
    .done(function(data, status){
        ...
        if(localStorage.getItem("idmodulspesial") == null){
            console.log("Hello from if")
            var appurl = localStorage.getItem("url_elearning_gsm")
            var token_user = getCookie("token_login_user_gsm")
            $.ajax({
            type: 'GET',
            url :appurl+"v1/modul-spesial", 
            headers : {
                "Authorization" : "Bearer "+token_user , 
                "Accept" : "application/json"
            }
            ...
            .done(function(data, status){
                var id_modul = data.data[0]._id
                localStorage.setItem("idmodulspesial",id_modul )
                var modulidcheck = checkScored(modul_status.scored)
                
                if(modulidcheck == localStorage.getItem("idmodulspesial")){
                    document.getElementById('aksesBasic').style.display ="none"
                    document.getElementById('listMateri').style.display ="block"
                }else{
                    document.getElementById('aksesBasic').style.display ="block"
                    document.getElementById('listMateri').style.display ="none"
                }
            })
        ...
        var modulidcheck = checkScored(modul_status.scored)
        var idmodulspesial = localStorage.getItem("idmodulspesial") 
        if(modulidcheck == idmodulspesial){
            document.getElementById('aksesBasic').style.display ="none"
            document.getElementById('listMateri').style.display ="block"
        }else{
            document.getElementById('aksesBasic').style.display ="block"
...
function upload_tantangan(){
    ...
    .done(function(status){
        ...
        var url_string = window.location.href
                ...
                if(kategori == 'spesial'){
                    window.location="detailmateri?kategori=spesial"
                }
                else{
                    window.location="detailmateri"
                }
...
function showthread2users(appurl, token_user, assesorId2){
    $.ajax({
        ...
        .done(function(response){
            ...
            $(".conversation-list").stop().animate({ scrollTop: $(".conversation-list")[0].scrollHeight}, 1000);
        })
